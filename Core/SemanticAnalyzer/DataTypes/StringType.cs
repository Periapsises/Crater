    ...
    public override Result TryLogicOperation(Value left, Value right, string op)
    {
        if (left.Kind == ValueKind.String && right.Kind == ValueKind.String)
        {
            var value = op switch
            {
                ...
                "__lt" => string.Compare(left.GetString(), right.GetString(), StringComparison.CurrentCultureIgnoreCase) < 0,
                "__le" => string.Compare(left.GetString(), right.GetString(), StringComparison.CurrentCultureIgnoreCase) <= 0,
